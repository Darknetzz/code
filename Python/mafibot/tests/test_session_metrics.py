...
def test_session_metrics_roundtrip(tmp_path, monkeypatch):
    ...
    assert finished.action_counts == {"crime": 2, "travel": 1}
    assert finished.rank_points_gained == 150
    loaded = sm.load_last_session_summary()
    assert loaded and loaded.profile == "ranker"
    assert loaded.money_end == 1000
    assert loaded.action_counts["crime"] == 2
...
def test_record_action_and_rank_gained():
    ...
    m.rank_start = 50_000
    m.rank_end = 50_420
    assert m.action_counts == {"crime": 2}
    assert m.rank_points_gained == 420
    data = m.to_dict()
    assert data["rank_points_gained"] == 420
    assert data["action_counts"]["crime"] == 2
