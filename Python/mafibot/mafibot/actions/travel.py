...
class TravelAction:
    ...
    async def run(
        ...
        *,
        ...
        await goto_side(page, "travel", policy=policy)
        await page_reading_pause(page)
        clicked = await click_button_matching(page, TRAVEL_ACTION_LABELS, policy=policy)
        await between_actions(page, policy)
        if clicked:
            return ActionResult(True, "travel action submitted")
        return ActionResult(False, "no travel button found")
