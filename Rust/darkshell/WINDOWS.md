# Darkshell (`dsh`) on Windows vs Unix

## Signals and interrupts

- **Unix / Linux**: When running an external command, Darkshell installs a `Ctrl+C` handler that requests the running child process group to terminate. At the interactive prompt, `rustyline` handles line editing; interrupt during prompt cancels the current line.
...
- Bash-style job control (`Ctrl+Z`, `fg`, `bg`, `%` job ids) is **not implemented** on any platform in v0. Background `&` starts a detached child on a best-effort basis; do not rely on POSIX job control semantics on Windows.
