...
pub fn endian_swap<P: AsRef<Path>>(input_file: P, output_file: P) {
    let mut in_file = fs::File::open(&input_file)
        .unwrap_or_else(|e| panic!("failed to open the ROM as a file: {}", e));
    let rom_len = in_file.metadata().unwrap().len();
    ...
    for idx in (0..rom_len).step_by(65536) {
        let remaining = (rom_len - idx).min(65536) as usize;
        let mut buf: Vec<u8> = vec![0; remaining];
        in_file.seek(SeekFrom::Start(idx)).unwrap();
...
pub fn byte_swap<P: AsRef<Path>>(input_file: P, output_file: P) {
    let mut in_file = fs::File::open(&input_file).unwrap();
    let rom_len = in_file.metadata().unwrap().len();
